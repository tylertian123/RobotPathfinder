...
namespace rpf {
    ...
    Path::Path(const std::vector<Waypoint> &waypoints, double alpha, PathType type)
        ...
        case PathType::CUBIC_HERMITE:
            for (size_t i = 0; i < waypoints.size() - 1; i++) {
                segments.push_back(std::make_unique<CubicSegment>(static_cast<Vec2D>(waypoints[i]),
                        static_cast<Vec2D>(waypoints[i + 1]),
                        Vec2D(std::cos(waypoints[i].heading) * alpha,
                                std::sin(waypoints[i].heading) * alpha),
                        Vec2D(std::cos(waypoints[i + 1].heading) * alpha,
                                std::sin(waypoints[i + 1].heading) * alpha)));
    ...
    void Path::update(double t, const Vec2D &p, const Vec2D &v, const Vec2D &a) {
        if (waypoints.size() > 2) {
            throw std::invalid_argument(
                    "update() is not supported on paths with multiple segments!");
        }
        ...
        segments[0] = std::make_unique<QuinticSegment>(p, static_cast<Vec2D>(waypoints[1]), v,
                Vec2D(std::cos(waypoints[1].heading) * alpha,
                        std::sin(waypoints[1].heading) * alpha),
                a, Vec2D(0, 0), t);
    }
} // namespace rpf
