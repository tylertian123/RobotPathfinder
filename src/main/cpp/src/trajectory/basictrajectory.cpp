...
namespace rpf {
    ...
    BasicTrajectory::BasicTrajectory(const RobotSpecs &specs, const TrajectoryParams &params) : specs(specs), params(params) {
        // Make the path
        ...
        // ds is the difference in the fraction of the total path length travelled for each iteration
        double ds = 1.0 / (params.sample_count - 1);
        double total = path->compute_len(params.sample_count);
        // dpi stands for Distance Per Iteration, it is the distance travelled along the path for each iteration
        ...
        for(int i = 1; i < params.sample_count; i ++) {
            ...
            // Since the additional velocity constraints are sorted from shortest path length to longest, we can 
            // check if we just surpassed one to determine whether we're on the point. Then, remove it so the 
            // process still works.
            ...
            if(moments[i - 1].vel < mv[i]) {
                ...
                double vel;
                if(maxv > mv[i]) {
                    // If it's more than the max then calculate the acceleration needed to reach the max
                    double accel = (mv[i] * mv[i] - moments[i - 1].vel * moments[i - 1].vel) / (2 * dpi);
                    vel = mv[i];
                    moments[i - 1].accel = accel;
                }
                else {
                    // Otherwise set the velocity to be the max and set the previous moment's acceleration
                    vel = maxv;
        ...
        for(size_t i = moments.size() - 1; i --> 0; ) {
            ...
            if(moments[i].vel > moments[i + 1].vel) {
                ...
                // If the velocity from the forwards pass is possible, then just set the acceleration
