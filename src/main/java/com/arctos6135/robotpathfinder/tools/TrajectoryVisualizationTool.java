...
public class TrajectoryVisualizationTool {
	...
	static void saveAsJson(String path, double maxVel, double maxAccel, double baseWidth, double alpha, 
			int sampleCount, PathType pathType) throws IOException {
    ...
    static void loadJson(File file) throws IOException {
        try(BufferedReader in = new BufferedReader(new FileReader(file))) {
            ...
            try {
				...
				JOptionPane.showMessageDialog(mainFrame, "Error: The file format is invalid.", 
        ...
        try(BufferedReader in = new BufferedReader(new FileReader(file))) {
            ...
            while((line = in.readLine()) != null && !line.equals("")) {
				...
				Waypoint w = new Waypoint(Double.parseDouble(point[0]), Double.parseDouble(point[1]), 
						Math.toRadians(constrainAngle(Double.parseDouble(point[2]))), point.length < 4 ? Double.NaN
			...
			do {
				...
				try {
                    ...
                    // If a row is selected then insert the row and the waypoint at the correct location
                        ...
                        waypoints.add(selectedRow, new Waypoint(x, y, Math.toRadians(constrainAngle(heading)), velocity));
		...
		saveMenuItem.addActionListener(e -> {
			...
			if(ret == JFileChooser.APPROVE_OPTION) {
                ...
                try {
                    ...
                    e1.printStackTrace();
                    JOptionPane.showMessageDialog(mainFrame, "Failed to save data!", "Error", JOptionPane.ERROR_MESSAGE);
                }
			}
		});
